...
$('#list_pasien').click(function() {
    ...
    $('#btn-print').click(function(e) {
        ...
        window.open("<?= base_url('print_gigi/') ?>" + no_struck, 'Cetak SOAP',
            'Cetak SOAP');
    })
...
$('#btn_tambah_gigi').click(function() {
    var elemen_gigi = $('#elemen_gigi').val()
    if (elemen_gigi != '' && elemen_gigi != null) {
        var output = `<tr>
                        <td class='elemen_gigi'>` + elemen_gigi + `</td>
                        <td><input type='hidden' class='depo' value='` + elemen_gigi +
            `'><button type='button' class='btnHapusGigi btn btn-danger btn-sm'>-</button></td>
                      </tr>`;
        $('#table_gigi tbody').append(output)
    }
})

$("#table_gigi").on('click', '.btnHapusGigi', function() {
...
$(document).on('click', '.btn_pilih_pasien', function() {
    ...
    $.ajax({
        type: "GET",
        url: '<?= base_url('gigi/get_soap') ?>' + '/' + no_struck,
        dataType: "JSON",
        beforeSend: function() {
            $('#table_gigi tbody').empty();
        },
        success: function(res) {
            if (res.status == 200) {
                ...
                $.each(res.data.elemen_gigi, function(key, value) {
                    var output = `<tr>
                        <td class='elemen_gigi'>` + value + `</td>
                        <td><input type='hidden' class='depo' value='` + value +
                        `'><button type='button' class='btnHapusGigi btn btn-danger btn-sm'>-</button></td>
                      </tr>`;
                    $('#table_gigi tbody').append(output)
                });
...
$('#btn-simpan-soap').click(function() {
    ...
    $('#table_gigi .elemen_gigi').each(function() {
        elemen_gigi.push($(this).text());
    });
    ...
    let param = {
        ...
        tindakan: tindakan,
        rujuk: rujuk,
        elemen_gigi: elemen_gigi,
        kode_poli: '002'
    }

    $.ajax({
        type: "POST",
        url: "<?= base_url('gigi/insert_soap') ?>",
        data: param,
        dataType: "json",
        beforeSend: function() {
            $('#loadingModal').show()
