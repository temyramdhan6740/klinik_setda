...
class Soap_umum extends MX_Controller
{
    ...
    public function get_pasien_by_struck($struck)
    {
        $data = $this->soap->get_pasien_by_struck($struck);
        if (isset($data->no_rm)) {
            echo json_encode(array('status' => 200, 'data' => $data));
        } else {
            echo json_encode(array('status' => 0, 'data' => null));
    ...
    public function print($no_struck)
    {
        ...
        if (isset($data->no_struck)) {
            $param['data'] = $data;
            $this->load->view('soap/print_soap_umum', $param);
        }
    }
